...
const CheckoutPage = () => {
    ...
    return (
        <section className="bg-[#FFF8E1] text-[#9B1C1C] min-h-screen flex items-center justify-center p-6">
            <div className="container max-w-screen-lg mx-auto">
                <div className="bg-white rounded shadow-lg p-6 md:p-12">
                    <h2 className="font-semibold text-2xl mb-4">Cash On Delivery</h2>
                    <p className="mb-2">Total Price: <span className="font-bold">${totalPrice}</span></p>
                    <p className="mb-6">Items: {cartItems.length > 0 ? cartItems.length : 0}</p>

                    <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 gap-y-6 text-sm grid-cols-1 lg:grid-cols-2">
                        <div>
                            <p className="text-lg font-bold mb-2">Personal Details</p>
                            <p>Please fill out all the fields.</p>
                        </div>
                        <div className="grid gap-4 text-sm grid-cols-1 md:grid-cols-2">
                            <div className="md:col-span-2">
                                <label>Full Name</label>
                                <input {...register("name", { required: true })} type="text" className="h-10 border rounded px-4 w-full bg-gray-100" />
                            </div>
                            <div className="md:col-span-2">
                                <label>Email Address</label>
                                <input type="text" disabled defaultValue={currentUser?.email} className="h-10 border rounded px-4 w-full bg-gray-100" />
                            </div>
                            <div className="md:col-span-2">
                                <label>Phone Number</label>
                                <input {...register("phone", { required: true })} type="number" className="h-10 border rounded px-4 w-full bg-gray-100" />
                            </div>
                            <div className="md:col-span-2">
                                <label>City</label>
                                <input {...register("city", { required: true })} type="text" className="h-10 border rounded px-4 w-full bg-gray-100" />
                            </div>
                            <div className="md:col-span-1">
                                <label>State</label>
                                <input {...register("state", { required: true })} type="text" className="h-10 border rounded px-4 w-full bg-gray-100" />
                            </div>
                            <div className="md:col-span-1">
                                <label>Zipcode</label>
                                <input {...register("zipcode", { required: true })} type="text" className="h-10 border rounded px-4 w-full bg-gray-100" />
                            </div>
                            <div className="md:col-span-2">
                                <label>Country</label>
                                <input {...register("country", { required: true })} type="text" className="h-10 border rounded px-4 w-full bg-gray-100" />
                            </div>
                            <div className="md:col-span-2 mt-4 flex items-center">
                                <input onChange={(e) => setIsChecked(e.target.checked)} type="checkbox" className="mr-2" />
                                <label>I agree to the <Link className='underline text-[#9B1C1C]'>Terms & Conditions</Link> and <Link className='underline text-[#9B1C1C]'>Shopping Policy</Link>.</label>
                            </div>
                            <div className="md:col-span-2 text-right mt-6">
                                <button disabled={!isChecked} className="bg-[#9B1C1C] hover:bg-[#B85C38] text-white font-bold py-3 px-6 rounded-md">Place an Order</button>
                            </div>
                        </div>
                    </form>
                </div>
